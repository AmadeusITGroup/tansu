...
interface Graph {
  sources: WritableSignal<number>[];
  layers: ReadableSignal<number>[][];
}
...
function makeGraph(config: TestConfig): GraphAndCounter {
  ...
  const sources = new Array(width).fill(0).map((_, i) => writable(i));
...
function runGraph(graph: Graph, iterations: number, readFraction: number): number {
  ...
  const sum = readLeaves.reduce((total, leaf) => leaf() + total, 0);
  return sum;
}
...
function makeRow(
  ...
  return sources.map((_, myDex) => {
    const mySources: ReadableSignal<number>[] = [];
